...
import { GetPermissionPreRoleAndModule, GetSystemModuleById } from '../../controllers/System/ModuleController';
      ...
      >
        { () => (
          <Form className='row'>
            ...
            <h5>Permisos</h5>
            <div className="px-3">
              { data?.permisionsByRol?.map( (item, index) => 
                <div className="row-placeholder mb-3" key={index} >
                  <h6>{ item[`roleName_${index + 1}`] }</h6>
                  <div className="content-col-between">
                    ...
                    <FieldController 
                      type="switch"
                      label="Crear"
                      name={`create_${index + 1}`} 
                    />
                    <FieldController 
                      type="switch"
                      label="Editar"
                      name={`update_${index + 1}`} 
                    />
                    <FieldController 
                      type="switch"
                      label="Eliminar"
                      name={`delete_${index + 1}`} 
                    />
                    <FieldController 
                      type="switch"
                      label="Importar"
                      name={`import_${index + 1}`}  
                    />
                    <FieldController 
                      type="switch"
                      label="Exportar"
                      name={`export_${index + 1}`}  
                    />
                    <FieldController 
                      type="switch"
                      label="Acceso web"
